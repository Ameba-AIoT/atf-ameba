...
if [ "$IMAGE_FILENAME" == "km0_image2_all.bin" ] || [ "$IMAGE_FILENAME" == "km4_image2_all.bin" ]; then
	...
	$ELF2BIN cert cert.json key_cert.json $KM4_IMG_DIR/cert.bin 0 key_img2.json 1 key_img3.json 2 key_ca7.json
...
if [ "$IMAGE_FILENAME" == "ap_image_all.bin" ]; then
	$ELF2BIN manifest manifest_ca7.json key_ca7.json $CA7_IMG_DIR/ap_image_all.bin $CA7_IMG_DIR/manifest.bin
	cat $CA7_IMG_DIR/manifest.bin $CA7_IMG_DIR/ap_image_all.bin > $CA7_IMG_DIR/ap_image_all_tmp.bin
	mv $CA7_IMG_DIR/ap_image_all_tmp.bin $CA7_IMG_DIR/ap_image_all.bin
fi
